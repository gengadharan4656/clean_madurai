  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<UserModel?>(
      ...
      builder: (context, userSnap) {
        ...
        return Scaffold(
          appBar: AppBar(title: Text('Collector Queue • $ward')),
          body: StreamBuilder<List<ComplaintModel>>(
            ...
            builder: (context, snap) {
              ...
              if (list.isEmpty) {
                return const Center(child: Text('No pending complaints in your ward.'));
              }
...
class _CollectorComplaintCardState extends State<_CollectorComplaintCard> {
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Text('${c.category} • #${c.id}', style: const TextStyle(fontWeight: FontWeight.w700)),
            Text('Status: ${c.status.replaceAll('_', ' ')}'),
            Text('Location: ${c.lat.toStringAsFixed(4)}, ${c.lng.toStringAsFixed(4)}'),
  ...
  Future<void> _markStatus(String status) async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text(ok ? 'Updated to $status' : (svc.lastError ?? 'Update failed'))),
    );
  }

  Future<void> _resolveWithAfterImage() async {
    final picked = await ImagePicker().pickImage(source: ImageSource.camera, imageQuality: 80, maxWidth: 1080);
    ...
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text(ok ? 'Complaint resolved and photo uploaded' : (svc.lastError ?? 'Resolve failed'))),
    );
  }
}
        ...
        child: Column(
          ...
          children: [
            SwitchListTile(
              ...
              subtitle: const Text('Used to notify citizens when collector is within 100m'),
              onChanged: (v) async {
                setState(() => _sharing = v);
                if (_pos == null || uid.isEmpty) return;
                await FirebaseFirestore.instance.collection('worker_live').doc(uid).set({
                  'uid': uid,
                  'lat': _pos!.latitude,
                  'lng': _pos!.longitude,
            ...
            const SizedBox(height: 8),
            const Text('Markers preview', style: TextStyle(fontWeight: FontWeight.w700)),
            const SizedBox(height: 8),
            _markerCard('📍 User marker', _pos == null ? 'Unknown' : '${_pos!.latitude.toStringAsFixed(4)}, ${_pos!.longitude.toStringAsFixed(4)}'),
            _markerCard('🗑️ Dustbin markers', 'Load from `dustbins` collection'),
            _markerCard('🚛 Worker live marker', 'From `worker_live` collection'),
            const SizedBox(height: 16),
            Expanded(
              child: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
                stream: FirebaseFirestore.instance.collection('dustbins').limit(20).snapshots(),
                builder: (context, snap) {
                  final bins = snap.data?.docs ?? [];
                  if (bins.isEmpty) {
                    return const Center(child: Text('No dustbin points found. Add docs in Firestore `dustbins` collection.'));
                  }
                  return ListView.builder(
                    ...
                    itemBuilder: (_, i) {
                      ...
                      final distance = _pos == null
                          ? null
                          : Geolocator.distanceBetween(_pos!.latitude, _pos!.longitude, lat, lng);
                      return ListTile(
                        leading: const Text('🗑️'),
                        title: Text(name),
                        subtitle: Text('${lat.toStringAsFixed(4)}, ${lng.toStringAsFixed(4)}'),
                        trailing: distance == null
                            ? null
                            : Text('${distance.round()} m', style: TextStyle(color: distance <= 100 ? Colors.green : Colors.grey)),
                      );
                    },
                  );
                },
        ...
        builder: (context, snap) {
          ...
          return ListView(
            ...
            children: [
              ...
              ListTile(title: const Text('Ward'), subtitle: Text(u?.ward ?? '-')),
              ListTile(title: const Text('Reward Points'), subtitle: Text('${u?.cleanlinessScore ?? 0}')),
